    ...
    >
      <el-container class="card-container">
        ...
        <el-main class="quotation-main">
          <el-row v-if="materialDeliveryCartonData.length">
            <el-col :span="4" v-for="(item, index) in tableData" :key="index">
              <el-card>
                ...
                <img
                  ...
                  :src="lostImg"
                  class="quotation-image"
                  @click="updateMaterial(item, dialog.option)"
                />
...
export default {
  ...
  watch: {
    dialog() {
      this.getDeliveryCartonData(this.dialog.option)
    }
  },
  methods: {
    // **********************************************  讀取資料開始 **********************************************
    getDeliveryCartonData(option) {
      this.$axios
        .get(`/api/material/${option}`)
        .then((res) => {
          // 獲取成功
          this.$message({
            message: `讀取 原物料中的 ${option} 類 資料完成`,
            type: 'success'
          })
          this.materialDeliveryCartonData = res.data
          this.setPaginations()
        })
        .catch((err) => {
    ...
    // **********************************************  分頁開始 **********************************************
    setPaginations() {
      this.my_paginations.total = this.materialDeliveryCartonData.length
      ...
      // }
      // 設置分頁數據
      this.tableData = this.materialDeliveryCartonData.filter((item, index) => {
    ...
    handleCurrentChange(page) {
      ...
      for (let i = index; i < nums; i++) {
        if (this.materialDeliveryCartonData[i]) {
          tables.push(this.materialDeliveryCartonData[i])
        }
    ...
    // option = paper or ink
    updateMaterial(material, option) {
      // 選擇完物件後 $emit 到父元件
      this.$emit('update', material, option)
    },
    // 時間轉換
    getDate(dt) {
