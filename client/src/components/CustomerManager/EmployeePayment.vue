<template>
  <div class="my-dialog">
      ...
      <div class="form">
        <el-container>
          ...
          <el-aside width="50%" class="grid-content bg-purple-light">
            ...
            >
              <!-- 這邊開始新增 -->
              <el-form-item prop="type" label="付款方式代碼：" size="mini">
                <el-input type="type" v-model="formData.type"></el-input>
              </el-form-item>
              <el-form-item prop="name" label="付款方式：" size="mini">
    ...
    <el-dialog
      title="編輯權限內容"
      :visible.sync="employeePaymentDialog"
      width="25%"
    >
      <el-form
        ref="editForm"
        :model="employeePaymentForm"
        :rules="employeePaymentFormRules"
        label-width="120px"
        style="margin:10px;width:auto"
      >
        <el-form-item
        ...
        >
          <el-input
            v-model="employeePaymentForm.type"
            autocomplete="off"
            size="mini"
            placeholder="D0001"
          ></el-input>
        ...
        >
          <el-input
            v-model="employeePaymentForm.name"
            autocomplete="off"
            size="mini"
            placeholder="請輸入中文名稱"
          ></el-input>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="employeePaymentDialog = false">取消</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      // 管理分頁
      my_paginations: {
        localStorage_page_size: 'employee_payment_page_size',
  ...
  methods: {
    // 分頁開始
    setPaginations() {
      this.my_paginations.total = this.employeePaymentData.length
      ...
      // 設置分頁數據
      this.tableData = this.employeePaymentData.filter((item, index) => {
    ...
    handleCurrentChange(page) {
      ...
      for (let i = index; i < nums; i++) {
        if (this.employeePaymentData[i]) {
          tables.push(this.employeePaymentData[i])
        }
    ...
    // 新增、編輯、刪除 第一層的分類
    handleAdd(form) {
      this.employeePaymentForm.option = 'add'
      this.onSubmit(form)
    },
    handleEdit(row) {
      // 第一層的資料
      this.employeePaymentForm.type = row.type
      this.employeePaymentForm.name = row.name
      this.employeePaymentForm._id = row._id
      this.employeePaymentForm.option = 'edit'
      this.employeePaymentDialog = true
    },
    handleDelete(row) {
      // 讓全部分類無法刪除
      MessageBox.confirm(
        ...
        .then(() => {
          this.$axios
            .delete(`/api/employee/payment/delete/${row._id}`)
            .then((res) => {
              this.$message('刪除成功！')
              this.$emit('update')
            })
      ...
      this.$refs[form].validate((valid) => {
        if (valid && !uploadFormData.type == '') {
          const url =
            this.employeePaymentForm.option == 'add'
              ? 'add'
              : `edit/${this.employeePaymentForm._id}`
          this.$axios
            .post(`/api/employee/payment/${url}`, {
              employeePayment: uploadFormData
            })
            .then((res) => {
              // 添加成功
              this.$message({
              ...
              this.employeePaymentDialog = false
