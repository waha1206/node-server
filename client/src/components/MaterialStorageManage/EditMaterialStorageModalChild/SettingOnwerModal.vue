<template>
  <div
    ...
    @click.stop="closeSettingOnwerModal"
  >
    <!-- 主體 -->
    <div
...
export default {
  ...
  data() {
    return {
      ...
      ptah: 'setting_onwer_modal_page_size', // 此業的顯示頁面儲存
  ...
  watch: {
    visible: {
      handler(val) {
        let el = document.getElementById('setting-onwer-modal')
  ...
  methods: {
    ...
    selectSupplier(supplier) {
      this.$emit('setting-onwer', supplier)
      this.$nextTick(() => {
        this.closeSettingOnwerModal()
      })
    },

    // 關閉視窗
    closeSettingOnwerModal() {
