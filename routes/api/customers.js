...
// 引入 User 才可以做查詢
const { Customer, CustomerClass, CustomerTitle } = require('../../models/Customer')
...
router.post('/class/add', passport.authenticate('jwt', { session: false }), (req, res) => {
...
router.post('/class/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  const customerFields = {}
  console.log(req.params)
  if (req.body.type) customerFields.type = req.body.type
  if (req.body.name) customerFields.name = req.body.name

  CustomerClass.findByIdAndUpdate({ _id: req.params.id }, { $set: customerFields }, { new: false }).then((customer) =>
    res.json(customer)
  )
})
...
router.delete('/class/delete/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
router.post('/title/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  CustomerTitle.findByIdAndUpdate({ _id: req.params.id }, { $set: customerTitleFields }, { new: false }).then(
    (customerTitles) => res.json(customerTitles)
  )
})
...
router.post('/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  Customer.findByIdAndUpdate({ _id: req.params.id }, { $set: customerFields }, { new: false }).then((customer) =>
    res.json(customer)
  )
})
