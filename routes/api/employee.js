...
// 引入 User 才可以做查詢
const { EmployeeAuthority, EmployeeJobClass, EmployeeJobTitle, EmployeePayment } = require('../../models/Employee')
...
router.post('/authority/add', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeeAuthority = _.cloneDeep(req.body.employeeAuthority)

  EmployeeAuthority.findOne({ name: employeeAuthority.name }).then((employeeAuthorityData) => {
...
router.post('/authority/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeeAuthority = _.cloneDeep(req.body.employeeAuthority)

  EmployeeAuthority.findByIdAndUpdate({ _id: req.params.id }, { $set: employeeAuthority }, { new: false }).then(
    (employeeAuthority) => res.json(employeeAuthority)
  )
})
...
router.delete('/authority/delete/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// 使用 hander 要驗證 token
// 使用 body 要放創建的資料 type name
router.post('/job-class/add', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeeJobClass = _.cloneDeep(req.body.employeeJobClass)

  EmployeeJobClass.findOne({ name: employeeJobClass.name }).then((employeeJobClassData) => {
...
router.post('/job-class/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeeJobClass = _.cloneDeep(req.body.employeeJobClass)
  EmployeeJobClass.findByIdAndUpdate({ _id: req.params.id }, { $set: employeeJobClass }, { new: false }).then(
    (employeeJobClassData) => res.json(employeeJobClassData)
  )
})
...
// 使用 hander 要驗證 token
// body 要放編輯的資料 key:value
router.delete('/job-class/delete/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// 使用 hander 要驗證 token
// 使用 body 要放創建的資料 type name
router.post('/job-title/add', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeeJobTitle = _.cloneDeep(req.body.employeeJobTitle)

  EmployeeJobTitle.findOne({ name: employeeJobTitle.name }).then((employeeJobTitleData) => {
...
// 有看到 post 就代表他會使用到 body 傳遞 數據 {}
// 有看到 /:id 就代表要從 params 接收一個 id 進來
router.post('/job-title/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeeJobTitle = _.cloneDeep(req.body.employeeJobTitle)

  EmployeeJobTitle.findByIdAndUpdate({ _id: req.params.id }, { $set: employeeJobTitle }, { new: false }).then(
    (employeeJobTitleData) => res.json(employeeJobTitleData)
  )
})
...
// 使用 hander 要驗證 token
// body 要放編輯的資料 key:value
router.delete('/job-title/delete/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
...
// 使用 hander 要驗證 token
// 使用 body 要放創建的資料 type name
router.post('/payment/add', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeePayment = _.cloneDeep(req.body.employeePayment)

  EmployeePayment.findOne({ name: employeePayment.name }).then((employeePaymentData) => {
...
// 有看到 post 就代表他會使用到 body 傳遞 數據 {}
// 有看到 /:id 就代表要從 params 接收一個 id 進來
router.post('/payment/edit/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  const employeePayment = _.cloneDeep(req.body.employeePayment)

  EmployeePayment.findByIdAndUpdate({ _id: req.params.id }, { $set: employeePayment }, { new: false }).then(
    (employeePaymentData) => res.json(employeePaymentData)
  )
})
...
// 使用 hander 要驗證 token
// body 要放編輯的資料 key:value
router.delete('/payment/delete/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
